...
void C_Radio::Send( )
{
    ...
    while ( true )
    {
        ...
        if (( m_IsOpen )&&( GetReceivedMode() == 0 ))
        {
            ...
            if (m_Log) fprintf(LogFile,"%d >> %03d %03d %03d %03d\n" , (int) timeGetTime() , (int) (m_LevelA + m_TrimA) , (int) ( m_LevelE + m_TrimE ) ,(int) ( m_LevelR + m_TrimR ) , (int) ( m_LevelT + m_TrimT ) ) ;
        }
...
void C_Radio::Listen( )
{
    ...
    while ( true )
    {
        ...
        if ( m_IsOpen )
        {
            ...
            if (m_Log) fprintf(LogFile,"%d << %d %03d %03d %03d %03d\n" , (int) timeGetTime(), (int)m_Received_Mode , (int) m_Received_LevelA , (int) m_Received_LevelE ,(int)  m_Received_LevelR , (int) m_Received_LevelT ) ;
        }
        pthread_mutex_unlock ( & m_RadioMutex ) ;
        Sleep(10) ;
    }
