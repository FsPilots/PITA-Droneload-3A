...
{
    char tmpstr[128] ;
    MyFrontCamera.set_size_accroche_error( (int) Slider3->GetValue()/100. ) ;
...
void ReglagesFront::OnPaint(wxPaintEvent& event)
{
    ...
    sprintf(tmpstr,"%.3f",MyFrontCamera.get_size_accroche_error()) ;
    StaticText6->SetLabel( tmpstr ) ;
    Slider3->SetValue( int (MyFrontCamera.get_size_accroche_error() * 100.) ) ;

}

void ReglagesFront::OnButton1Click(wxCommandEvent& event)
{
    Close() ;
}
