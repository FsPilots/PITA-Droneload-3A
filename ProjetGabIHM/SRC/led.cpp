...
wxLed::wxLed (wxWindow * parent, wxWindowID id, wxColour disabledColor, wxColour enableColourOn, wxColour enableColourOff, const wxPoint & pos, const wxSize & size)
:
	...
	m_bitmap (NULL)
{
	...
	m_EnableOff = enableColourOff;
	m_EnableOn = enableColourOn;
	Enable();
}
...
void wxLed::SwitchOn (void)
{
	...
	this->SetBitmap (this->m_EnableOn.GetAsString( wxC2S_HTML_SYNTAX)) ;
}

void wxLed::SwitchOff (void)
{
	wxWindow::Enable (true) ;
	this->SetBitmap (this->m_EnableOff.GetAsString( wxC2S_HTML_SYNTAX)) ;
}
...
void wxLed::SetColorOn (wxColour rgb)
{
	m_EnableOn = rgb;
    if (this->m_isEnabled)
        this->SetBitmap (this->m_EnableOn.GetAsString( wxC2S_HTML_SYNTAX)) ;
...
void wxLed::SetColorOff (wxColour rgb)
{
    m_EnableOff = rgb;
    if (this->m_isEnabled)
        this->SetBitmap (this->m_EnableOff.GetAsString( wxC2S_HTML_SYNTAX)) ;
